...
2. **Gather context**
   ...
   - If a reference image exists but the provider does not support vision, do NOT retry vision_analyze/browser_vision. Instead: ask the user for a concise description (3-5 key details: colors, layout, style, elements, text), or proceed with a clean design and invite refinement.
...
- **Do not loop on vision analysis when the provider does not support it.** If `vision_analyze` or `browser_vision` fails with `unknown variant 'image_url'` or similar provider error, stop immediately. The model cannot see images — switch strategy: ask the user to describe the reference, or proceed with code-only changes.
- **When the user says it's "just CSS" or "just a simple fix", trust them.** Do not over-engineer by trying to analyze images, install tools, or investigate — the simplest path is the right one. Start with the direct fix, ask for refinement after.
- **Do not retry a failing tool more than 3 times in a row.** If the tool keeps failing with the same error type, the approach is wrong, not the parameters. Document the limitation and pivot.
