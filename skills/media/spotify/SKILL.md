...
- **Don't assume the browser opens automatically in WSL** during `hermes auth spotify` — `gio` is unavailable. Copy the URL from the terminal output instead. See `references/setup-pitfalls.md` for full WSL setup guidance.
